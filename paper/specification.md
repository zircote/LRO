...
description: "Token-aware result offloading: threshold detection, JSONL materialization, and guided consumption for context-efficient AI workflows."
---
...
> **Scope note:** `list_memories` is available as both an MCP tool and an HTTP API operation (`GET /v1/memories`). LRO applies at the MCP transport layer; HTTP clients SHOULD use the offset-based pagination (`limit` and `offset` parameters) defined in [HTTP API](/atlatl-spec/specification/http-api) instead.
...
Token estimation MUST use the same heuristic defined in [Prompt Integration - Context Window Budgeting](/atlatl-spec/specification/prompting#context-window-budgeting): `tokens ~ characters / 4` for Latin-script content, with model-specific tokenizers RECOMMENDED for CJK or mixed-script content.
...
See [Data Model - Memory](/atlatl-spec/specification/data-model#memory) for the complete field reference.
...
See [Custodial System - Standard Custodial Tasks](/atlatl-spec/specification/custodial#standard-custodial-tasks) for the registration pattern.
...
See [Configuration](/atlatl-spec/specification/configuration) for the full environment variable mapping conventions.

## Known Constraints

LRO is designed for the primary atlatl deployment model: a local MCP server connected via stdio to an LLM client with shell tool access. Two deployment configurations fall outside this model.
...
**Resolution:** The [Remote Transport Proxy](/atlatl-spec/specification/remote-transport-proxy) resolves this constraint. When a proxy client connects to a remote server, the remote server detects `clientInfo.proxy: true` and returns full results inline (bypassing its own LRO). The local proxy then applies its own LRO interceptor, writing JSONL files to the local filesystem where the LLM's shell tools can access them. See [Remote Transport Proxy - LRO Integration](/atlatl-spec/specification/remote-transport-proxy#lro-integration) for the complete protocol.

For deployments **without** a proxy, the original constraint still applies: streamable-http transport SHOULD disable LRO (`prompt.offload.enabled = false`) unless the client and server share a filesystem (e.g., network-mounted storage). When LRO is disabled, the standard inline response path applies: results are returned directly in the tool response, subject to the MCP tool's `limit` parameter and the token budgeting in [Prompt Integration](/atlatl-spec/specification/prompting).
...
1. The proxy connects to the remote atlatl server via streamable-http and registers itself as a proxy client (`clientInfo.proxy: true`).
2. The remote server, detecting the proxy flag, returns full inline results (bypassing its own LRO interceptor) as specified in [Remote Transport Proxy - LRO Integration](/atlatl-spec/specification/remote-transport-proxy#lro-integration).
3. The local proxy applies its own LRO interceptor to the received results, writing JSONL files to the local filesystem using the standard [JSONL File Format](#jsonl-file-format) and [File Naming](#file-naming) conventions.
...
When an implementation provides native extraction, the tool MUST generate its own `guidance` prompt that directs the LLM to the extraction MCP tool rather than to shell-based jq commands. The standard [Guidance Prompt](#guidance-prompt) template assumes shell access; native extraction replaces that assumption entirely. Implementations SHOULD use the following template:
...
- **No shell dependency.** The LLM does not need access to `bash`, `jq`, or `tail`. This extends LRO's utility to shell-less clients that would otherwise fall back to the summary-only path described in [Shell-less LLM Clients](#shell-less-llm-clients).
- **Reduced tool-call overhead.** A single MCP tool call replaces the multi-step sequence of formulating a shell command, executing it, and parsing the output. The LLM avoids reasoning about shell syntax entirely.
- **Sandboxed execution.** The extraction runs within the MCP server process with controlled file access, rather than in an unrestricted shell environment. Implementations MAY validate the `file_path` parameter against the configured `output_dir` to prevent path traversal.
- **Recipe parameterization.** The `params` object allows the LLM to parameterize standard recipes (e.g., filter by a specific namespace or keyword) without string interpolation in shell commands, eliminating a class of injection errors.
- **Self-contained prompt.** The tool generates its own guidance, so prompt content stays in sync with the tool's interface. No external coordination is needed to keep the guidance accurate as the tool evolves.

> **Implementation note:** The native extraction tool is fully compatible with the local LRO proxy architecture. A proxy that materializes JSONL files locally already has filesystem access and can trivially add an extraction tool. Standalone local MCP servers that connect to a remote atlatl server MAY implement both proxy materialization and native extraction in a single process.
...
The OffloadResponse remains partially useful without shell access. The `summary` field provides the result count, top namespaces, score range, and detail level. The LLM can use this metadata to answer high-level questions about the result set (e.g., "How many memories matched?", "Which namespaces are represented?") or to inform the user that detailed results are available in the offloaded file.

Implementations MAY detect the client's tool capabilities during MCP initialization and adjust behavior:

- If the client declares no tool-use capability beyond atlatl's own tools, the server MAY increase the LRO threshold or disable LRO to favor inline results.
...
See [Conformance Levels](/atlatl-spec/specification/conformance) for the complete conformance matrix.

## Cross-References

| Section | Relationship |
|---------|-------------|
| [Paginated Recall](/atlatl-spec/specification/paginated-recall) | Complementary mechanism; LRO takes precedence when result set exceeds threshold |
| [Prompt Integration](/atlatl-spec/specification/prompting) | Token budgeting heuristic shared with LRO threshold estimation |
| [Remote Transport Proxy](/atlatl-spec/specification/remote-transport-proxy) | Resolves LRO file accessibility for remote deployments |
| [Custodial System](/atlatl-spec/specification/custodial) | `offload_cleanup` task registration pattern |
| [Configuration](/atlatl-spec/specification/configuration) | `[prompt.offload]` configuration keys |
| [LRO Validation Experiment](/atlatl-spec/specification/lro-experiment) | Non-normative experiment plan for empirical validation of LRO assumptions |
